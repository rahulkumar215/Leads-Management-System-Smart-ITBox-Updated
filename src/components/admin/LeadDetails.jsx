      ...
      <Layout>
        <div className="row">
          <div className="col-lg-6">
            <h4 style={{ textTransform: "capitalize", marginBottom: "30px" }}>
              Company: {lead.companyName || "Unknown"}
            </h4>
            <p style={{ textTransform: "capitalize" }}>
              <strong>Status:</strong> {lead.status || "Draft"}
            </p>
            <p style={{ textTransform: "capitalize" }}>
              <strong>Data Analyst:</strong> {lead.createdBy || "N/A"}
            </p>
            <p style={{ textTransform: "capitalize" }}>
            ...
            <p style={{ textTransform: "capitalize" }}>
              <strong>Company Lost:</strong> {lead.isCompanyLost ? "Yes" : "No"}
            </p>
            <p style={{ textTransform: "capitalize" }}>
              <strong>Company Lost Reason:</strong> {lead.companyLostReason}
            </p>
            ...
            <div className="product_details custom_table">
              ...
              <p style={{ marginTop: "15px", fontSize: "18px" }}>
                ...
                {lead?.growthManagerPipeline?.reduce(
                  (sum, stage) => sum + (stage.totalPrice || 0),
                  0
                )}
              </p>
            </div>
              ...
              <div className="admin_completed_stage_showcase">
                ...
                <div>
                  {lead?.growthManagerPipeline
                    ...
                    ?.map((stage, index) => (
                      <div
                        className="stage_name_complete"
                        key={index}
                        style={{ alignItems: "center", display: "flex" }}
                      >
                        <IoIosCheckbox style={{ fontSize: "20px" }} />{" "}
                        <span style={{ marginLeft: "5px" }}>{stage.stage}</span>
                      </div>
              ...
              <div className="admin_completed_stage_showcase">
                {lead?.growthManagerPipeline
                  ?.filter(
                    (stage) =>
                      !stage.completed &&
                      new Date(stage.tatDeadline) < new Date()
                  )
                  ?.map((stage, index) => (
                    <div
                      key={index}
                      className="overdue-alert admin_showcase_alert"
                      style={{ display: "flex", alignItems: "center" }}
                    >
                      <AiFillCloseSquare
                        style={{ fontSize: "20px", marginRight: "5px" }}
                      />
                      {stage.stage} - Overdue by{" "}
                      {Math.ceil(
                        (new Date() - new Date(stage.tatDeadline)) /
                          (1000 * 60 * 60 * 24)
                      )}{" "}
                      days!
                    </div>
            ...
            {lead?.contactPoints?.map((person, index) => (
              <div className="admin_contact_person_card" key={index}>
                ...
                <span style={{ display: "block", padding: "5px 0px" }}>
                  ...
                  <strong>Alt: </strong> {person.alternateEmail || "N/A"}
                </span>
                <span style={{ display: "block", padding: "5px 0px" }}>
                  <strong>Phone:</strong> {person.phone} /{" "}
                  <strong>Alt: </strong> {person.alternatePhone}
                </span>
                <span style={{ display: "block", padding: "5px 0px" }}>
                  <strong>Whatsapp:</strong> {person.whatsappNumber}
                </span>
                <span style={{ display: "block", padding: "5px 0px" }}>
